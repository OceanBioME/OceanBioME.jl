...
function sugarkelpequations(A, N, C, NO₃, irr, t, params, Δt)
    ...
    return (A = dA / (60*60*24), N = dN / (60*60*24), C = dC / (60*60*24), j = (A+Δt*dA / (60*60*24)) * j / (60*60*24*14*0.001))
end
...
source_fields = (ParticleField(:NO₃, :NO₃, 1), ParticleField(PAR, PAR, 1))
sink_fields = (ParticleField(:j, :NO₃, -1), )
#need to change urel at some point
kelp_particles = BioLagrangianParticles(particles, sugarkelpequations, 
                                    (:A, :N, :C, :NO₃, :PAR), (urel=0.15, temp=temperature_itp, λ=λ_arr, 
                                    resp_model=2, paramset=SugarKelp.broch2013params), 
                                    (:A, :N, :C), 
                                    (:j, ), 
                                    source_fields,
                                    sink_fields,
