...
required_biogeochemical_tracers(::ExternalContractPlankton) =
    (:P1, :P2, :C1, :C2)

required_biogeochemical_auxiliary_fields(::ExternalContractPlankton) = tuple()

group_element_tracers(::ExternalContractPlankton, bgc, ::Val{:nitrogen}) =
    (P1 = 1.0, P2 = 1.0, C1 = 2.0, C2 = 2.0)

chlorophyll_ratio(::ExternalContractPlankton) = 1.31
chlorophyll_ratio(::ExternalContractPlankton, ::Val{:P2}) = 0.9
...
@inline nutrient_uptake(i, j, k, grid, plankton::ExternalContractPlankton, bgc::ExternalContractNPD{FT}, fields, auxiliary_fields) where FT = FT(0.4)
@inline solid_waste(i, j, k, grid, plankton::ExternalContractPlankton, bgc::ExternalContractNPD{FT}, fields, auxiliary_fields) where FT = FT(0.3)
@inline dissolved_waste(i, j, k, grid, plankton::ExternalContractPlankton, bgc::ExternalContractNPD{FT}, fields, auxiliary_fields) where FT = FT(0.2)
@inline inorganic_waste(i, j, k, grid, plankton::ExternalContractPlankton, bgc::ExternalContractNPD{FT}, fields, auxiliary_fields) where FT = FT(0.1)

@inline grazing(i, j, k, grid, ::Val{:sPOM}, plankton::ExternalContractPlankton, bgc::ExternalContractNPD{FT}, fields, auxiliary_fields) where FT = FT(0.5)
...
@testset "External plankton interface" begin
    ...
    @test nutrient_uptake(1, 1, 1, grid, plankton, bgc, fields, auxiliary_fields) == 0.4
    @test solid_waste(1, 1, 1, grid, plankton, bgc, fields, auxiliary_fields) == 0.3
    @test dissolved_waste(1, 1, 1, grid, plankton, bgc, fields, auxiliary_fields) == 0.2
    @test inorganic_waste(1, 1, 1, grid, plankton, bgc, fields, auxiliary_fields) == 0.1
    @test grazing(1, 1, 1, grid, Val(:sPOM), plankton, bgc, fields, auxiliary_fields) == 0.5
    @test chlorophyll_ratio(plankton, Val(:P1)) == 1.31
    @test chlorophyll_ratio(plankton, Val(:P2)) == 0.9
end
