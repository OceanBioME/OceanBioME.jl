using Oceananigans, OceanBioME, Oceananigans.Units, StructArrays
...
particle_properties = StructArray{CTracking}((zeros(n_particles), zeros(n_particles), zeros(n_particles), zeros(n_particles)))
...
simulation.output_writers[:velocities] = JLD2Writer(model, model.velocities, overwrite_existing = true, filename = "drogued_velocities.jld2", schedule = TimeInterval(10minutes))
...
simulation.output_writers[:particles] = JLD2Writer(model, (; particles), overwrite_existing = true, filename = "drogued_particles.jld2", schedule = TimeInterval(0.1minutes))
simulation.output_writers[:tracer] = JLD2Writer(model, model.tracers, overwrite_existing = true, filename = "drogued_tracer.jld2", schedule = TimeInterval(0.1minutes))
