...
@inline function (p::pCO₂)(DIC, Alk, T, S)
    ...
    k⁰ = exp(pk⁰.C + pk⁰.invT / T  + pk⁰.ClogT * log(T * pk⁰.logCT) + S * (pk⁰.S + pk⁰.ST * T + pk⁰.ST² * T ^ 2))
    k¹ = 10 ^ (pk¹.C + pk¹.invT / T + pk¹.logT * log(T) + pk¹.S * S + pk¹.S² * S ^ 2)
    k² = 10 ^ (pk².C + pk².invT / T + pk².S * S + pk².S² * S ^ 2 + pk².logT * log(T)) # added log(T) term
    ...
    pCO₂ = ((DIC * H ^ 2/ (H ^ 2 + k¹ * H + k¹ * k²))/k⁰)*10^6
    return pCO₂ # μatm
end
OCMIP_solubility = (C = -60.2409, invT = 93.4517 * 100, logCT = 1 / 100, ClogT = 23.3585, T² = 0, ST² = 0.0047036 / (100 ^ 2), ST = -0.023656 / 100, S = 0.023517)
