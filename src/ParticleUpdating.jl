module Particles
export BioLagrangianParticles, ParticleField
#export of structs is frowned upon I think

using KernelAbstractions, Oceananigans

@warn "Scalefactor is not currently implimented for property sourcing"
...
@kernel function property_update!(particles, equation::Function, arguments, params, update_properties, track_properties)
    p = @index(Global)

    @inbounds argument_values = [arg[p] for arg in arguments]

    results = equation(particles.properties.x, particles.properties.y, particles.properties.z, argument_values..., params, Δt)
...
function dynamics!(particles, model, Δt)
    ...
    for tracked_field in particles.parameters.tracked_fields
        ...
        particle_property = getproperty(model.particles.properties, tracked_field.property)
        ...
        source_event = update_field_property_kernel!(particle_property, model.particles.properties, model.grid, tracer, LX(), LY(), LZ())
        wait(source_event)
    end

    #update particle properties
    particles.properties.time .= time(simulation) #making this be a tracked field incase function calls need it
    function_arguments = getproperty.(model.particles.properties, model.particles.parameters.arguments)
    ...
    property_update_event = property_update_kernal!(model.particles, model.particles.parameters.equation, function_arguments, function_parameters, model.particles.parameters.equation_integrals, model.particles.parameters.equation_statics)
    ...
    sourcesink_event = sourcesink_kernal!(model, (LX(),LY(),LZ()), source_properties, source_scalefactors, source_scalefactors, Δt)
...
function BioLagrangianParticles(particles::StructArray, equation::Function, equation_arguments::Tuple{Symbol}, 
    equation_parameters, equation_integrals::Tuple{Symbol}, equation_statics::Tuple{Symbol}, tracked_fields::Tuple{ParticleField}, 
    sink_fields::Tuple{ParticleField}, density::AbstractFloat, custom_dynamics=no_dynamics)
