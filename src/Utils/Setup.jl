...
function Oceananigans(model::Symbol, 
    ...
    model, discrete_forcings, sinking_forcings, optional_tracers = loadmodel(model)
    ...
    for tracer in tracers.core
        forcing, bcs = setuptracer(model, grid, tracer, tracers.core, topboundaries, bottomboundaries, forcing_params, discrete_forcings, sinking_forcings; sinking=sinking, advection_scheme=advection_scheme, no_sinking_flux=no_sinking_flux)
    ...
    for optionset in tracers.optional
        for tracer in optionset
            forcing, bcs = setuptracer(model, grid, tracer, (tracers.core..., optionset...), topboundaries, bottomboundaries, forcing_params, discrete_forcings, sinking_forcings; sinking=sinking, advection_scheme=advection_scheme, no_sinking_flux=no_sinking_flux)
            forcing_functions = (forcing_functions..., forcing)
            boundary_functions = (boundary_functions..., bcs)
        end
    end
