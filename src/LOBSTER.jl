module LOBSTER
#=
The Lodyc Ocean Biogeochemical SimulationTools for Ecosystem and Resources (LOBSTER) model consists of six prognostic variables 
expressed in terms of their nitrogen content: 
nitrate (NO₃), ammonium (NH₄), phytoplankton (P), zooplankton (Z), detritus(D) and semilabile dissolved organic matter (DOM).
Current version has been expanded to include Large detritus (DD) and optional tracers: dissolved inorganic carbon (DIC), alkalinity (ALK) and oxygen (OXY). 
Details can be found in references: 
...
#source functions, refer to Table A1 in reference (1)
...
#detritus sinking speed profile 
