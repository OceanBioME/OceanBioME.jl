...
mutable struct PositiveQuasiAdamsBashforth2TimeStepper{FT, GT, IT} <: AbstractTimeStepper
                  χ :: FT
        previous_Δt :: FT
                 Gⁿ :: GT
                 G⁻ :: GT
    implicit_solver :: IT
  intermediate_step :: GT
end

"""
PositiveQuasiAdamsBashforth2TimeStepper(grid, tracers,
                                    χ = 0.1;
...
!!! note "First timestep"
    ...
    ```
"""
function PositiveQuasiAdamsBashforth2TimeStepper(grid, tracers,
                                         χ = 0.1;
                                         implicit_solver::IT = nothing,
                                         Gⁿ = TendencyFields(grid, tracers),
                                         G⁻ = TendencyFields(grid, tracers),
                                         Cⁿ⁺¹ = TendencyFields(grid, tracers)) where IT

    FT = eltype(grid)
    GT = typeof(Gⁿ)

    return PositiveQuasiAdamsBashforth2TimeStepper{FT, GT, IT}(χ, Inf, Gⁿ, G⁻, implicit_solver, Cⁿ⁺¹)
end

function reset!(timestepper::PositiveQuasiAdamsBashforth2TimeStepper)
...
"""
    time_step!(model::AbstractModel{<:PositiveQuasiAdamsBashforth2TimeStepper}, Δt; euler=false)
...
"""
function time_step!(model::AbstractModel{<:PositiveQuasiAdamsBashforth2TimeStepper}, Δt; callbacks = [], euler=false)
