...
@inline function (bgc::NitrateAmnmoniaPISCES)(i, j, k, grid, val_name::Val{:NO₃}, clock, fields)
    θ = bgc.nitrogen_redfield_ratio

    nitrif = nitrifcation(bgc.nitrogen, i, j, k, grid, bgc, clock, fields)

    remineralisation = oxic_remineralisation(bgc.dissolved_organic_matter, i, j, k, grid, bgc, clock, fields)
...
@inline function (bgc::NitrateAmnmoniaPISCES)(i, j, k, grid, val_name::Val{:NH₄}, clock, fields)
    θ = bgc.nitrogen_redfield_ratio

    nitrif = nitrifcation(bgc.nitrogen, i, j, k, grid, bgc, clock, fields)

    remineralisation = anoxic_remineralisation(bgc.dissolved_organic_matter, i, j, k, grid, bgc, clock, fields)

    consumption = uptake(bgc.phytoplankton, val_name, i, j, k, grid, bgc, clock, fields)

    grazing_waste = inorganic_excretion(bgc.zooplankton, i, j, k, grid, bgc, clock, fields)

    upper_trophic_waste = upper_trophic_respiration(bgc.zooplankton, i, j, k, grid, bgc, clock, fields)

    fixation = nitrogen_fixation(bgc.nitrogen, i, j, k, grid, bgc, clock, fields)

    return fixation + θ * (remineralisation + grazing_waste + upper_trophic_waste - consumption) - nitrif
end

@inline function nitrification(nitrogen, i, j, k, grid, bgc, clock, fields)
    λ = nitrogen.maximum_nitrifcation_rate

    O₂  = @inbounds fields.O₂[i, j, k]
    PAR = @inbounds fields.mean_mixed_layer_light[i, j, k]
...
@inline function nitrogen_fixation(nitrogen, i, j, k, grid, bgc, clock, fields)
    ...
    availability_limitation = nitrogen_availability_limitation(bgc.phytoplankton, i, j, k, grid, bgc, clock, fields)

    fixation_limit = ifelse(availability_limitation >= 0.8, 0.01, 1 - availability_limitation)

    μ = base_production_rate(bgc.phytoplankton, i, j, k, grid, bgc, clock, fields)
