...
"""
    GasExchangeBoundaryCondition(; wind_speed,
                                   water_concentration,
                                   air_concentration,
                                   transfer_velocity,
                                   grid = nothing,
...
with `transfer_velocity` and `wind_speed`.

`water_concentration`, `air_concentration`, and `wind_speed` can either be numbers, functions of the form
`(x, y, t)`, functions of the form `(i, j, grid, clock, model_fields)` if `discrete_form` is set to true,
or any kind of `Field` or `FieldTimeSeries`.
...
`transfer_velocity` should usually be a `SchmidtScaledTransferVelocity` provided in this module.
...
function CarbonDioxideGasExchangeBoundaryCondition(FT = Float64;
    ...
    return GasExchangeBoundaryCondition(FT; wind_speed, water_concentration, air_concentration, transfer_velocity, grid, discrete_form, kwargs...)
end
...
OxygenGasExchangeBoundaryCondition(FT = Float64;
                                   ...
                                   kwargs...) = # mmolO₂/m³
    GasExchangeBoundaryCondition(FT; wind_speed, water_concentration, air_concentration, transfer_velocity, grid, discrete_form, kwargs...)
