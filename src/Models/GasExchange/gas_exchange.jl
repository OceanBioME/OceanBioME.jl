...
`air_concentration` with a `transfer_velocity` computed from the `wind_speed` and
temperature.
...
`transfer_velocity`, `water_concentration`, `air_concentration` and `wind_speed`
can either be numbers, functions of the form `(x, y, t)`, functions of the form
`(i, j, grid, clock, model_fields)` if `discrete_form` is set to true, or any kind
of `Field`.
...
@inline mask_value(mask, i, j, grid, clock, fields) = surface_value(mask, i, j, grid, clock, fields)
...
summary(::GasExchange{WS, TV, WC, AC}) where {WS, TV, WC, AC} = "Air-sea `GasExchange` model for $(nameof(WC))"
