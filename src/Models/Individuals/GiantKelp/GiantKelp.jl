...
@kwdef struct GiantKelp{GR, PP, RR, NU, ML, FT}
                          growth :: GR = ReserveAmmoniaGrowth(; maximum_specific_growth_rate = 0.6, # per maximum growth from sbclter https://sbclter.msi.ucsb.edu/data/catalog/package/?package=knb-lter-sbc.108
                  ...
                  photosynthesis :: PP = JassbyAndPlatt(15 * 100 * 12 * 10^-6,
                                                        0.1 * 100 * 12 * 10^-6,# 
                                                        0.5, Inf)#ArrheniusPhotosynthesis() # TODO : update parameters from 474 V.A. Gerard: Photosynthetic characteristics of giant kelp
                     respiration :: RR = BasalAndUptakeRespiration(; base_basal_respiration_rate = 2.5 * 100 * 12 * 10^-6) # can't constrain
                 nitrogen_uptake :: NU = DroopUptake(; nitrate_half_saturation = 10.2, # per macmods
                                                       ...
                                                       maximum_nitrate_uptake = 752 * 10^-6 * 14 / 100 * 24, # per macmods -> gN / dm^2 / day
                                                       maximum_ammonia_uptake = 739 * 10^-6 * 14 / 100 * 24) # per macmods -> gN / dm^2 / day
                  mortality_loss :: ML = SigmoidMortality(; base_rate = (0.0391+0.0199)/2* 3)
  ...
  structural_dry_weight_per_area :: FT = 0.1 # from https://onlinelibrary.wiley.com/doi/epdf/10.1111/j.1529-8817.2006.00287.x assuming the minimums and structural content above
end
