...
defaults = (
    ...
    δ = (P = 0.05, D = 0.05),
    t_dark = (P = 3, D = 4), #time to death below euphotic layer
    ...
    Sᵣₐₜ = (P = 3, D = 3),
    Kₛᵢ¹ = 8e-6*1e5,#from NEMO source code, seem way way too small, all their other parameters seem to be 1e5 smaller so going to assume same messing with units
    Kₛᵢ² = 20e-6*1e5,
    ...
    m = (P = 0.01/day, D = 0.01/day, Z = 0.004/day, M = 0.03/day),
    wᴾ = 0.01/day, #agregation
    wₘₐₓᴰ = 0.03/day,
    Pₘₐₓ = (P = 1, D = 1),
    ...
    ## POC Iron uptake
    #NEMO has an additional factor of 0.6 in their BactFe calculation, and then anothert factor of 0.1 with all uses of it
    κᶠᵉᴾᴼ_Bact = 0.8, #all infered from NEMO source code
    κᶠᵉᴳᴼ_Bact = 0.2,
    θₘₐₓᶠᵉᴮᵃᶜᵗ = 6,#μmol Fe/mol C
    ...
    r_CaCO₃ = 0.3, 
    η = (Z = 0.3, M = 0.3), #from Jansen and Wolf-Gladrow 2001 https://www.int-res.com/articles/meps/221/m221p199.pdf
